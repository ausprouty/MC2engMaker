...
export default {
	...
	async addNote(noteid, route, noteText){
		var source = localStorage.getItem('mc2NoteSourceEng');
		if (source == 'database'){
	...
	async notesFromDatabase(route){
			try {
				...
				await sqlite.closeConnection("db_notes_eng");
				return res.values
			} catch (err) {
				console.log (' error in SQLite Service notes from database for ' + route)
				console.log(`Error: ${err}`);
				localStorage.removeItem('mc2NoteSourceEng')
	...
	async openDatabase(){
		try {
			...
			const isConn = (await sqlite.isConnection("db_notes_eng")).result;
			//console.log(`after isConnection ${isConn}`);
			let db;
			if (ret.result && isConn) {
				//console.log("I am retreiving connection")
				db = await sqlite.retrieveConnection("db_notes_eng");
			} else {
				//console.log("I am creating  connection")
				db = await sqlite.createConnection("db_notes_eng", false, "no-encryption", 1);
			}
			await db.open();
			return db
		} catch (err) {
  ...
  // Dealing with Textarea Height
	...
	async getDataSource(){
		var noteSource = localStorage.getItem('mc2NoteSourceEng')
		if (!noteSource){
	...
	async createDataStore(){
		...
		try {
			...
			if (res.changes && res.changes.changes && res.changes.changes < 0) {
				...
				localStorage.setItem('mc2NoteSourceEng', 'localstorage')
				return;
			}
			await sqlite.closeConnection("db_notes_eng");
			localStorage.setItem('mc2NoteSourceEng', 'database')
		} catch (err) {
			localStorage.setItem('mc2NoteSourceEng', 'localstorage')
		}
		return localStorage.getItem('mc2NoteSourceEng')
	}
}
